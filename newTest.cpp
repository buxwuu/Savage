...
int createUser(){
	...
	while (choice!="2"){
		...
		getline(cin, password);
		pstmt=con->prepareStatement("SELECT player    _ID FROM player_info WHERE username='"+username+"'");
		res=pstmt->executeQuery();
		if(res->first()){
...
void test(){
  ...
  istringstream iss("1 newTest myp4ssword 1 1 3 1 Executor 2 1"); //change the initial 1 to a 2 to login instead of making a new user.  Making a new user will only work once!
  cin.rdbuf(iss.rdbuf());
  while(cin >> choice >> username >> password >> instr >> instr1 >> instr2 >> instr3 >> instr4 || !cin.eof()) {
      if(cin.fail()) {
          cin.clear();
          string dummy;
          cin >> dummy;
