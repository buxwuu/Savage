...
int login(){
	...
	while (true){
		...
		pstmt=con->prepareStatement("SELECT player_ID FROM player_info WHERE username='"+username+"' AND 
			password='"+password+"'");
...
int createUser(){
	...
	while (choice!="2"){
		...
		if(res->first()){
			...
			delete pstmt;
			pstmt=con->prepareStatement("INSERT INTO player_info (username, password) VALUES ('"+username+"', '"
				+password+"')");
			pstmt->executeUpdate();
		}
		delete pstmt;
...
case 1:
    ...
    update2 = "A long time ago, before Tupac but not really before Tupac.  Luke Skywalker has returned to his 
    	home planet of Tatooine in order to. . .you know what I don't remember, it's been to long since I've 
    	...
    	on this one. . .just please do me a favor and lower your expectations, we promise we'll make it up to you as 
    	the game goes on.  We are all pressed for time in our other classes anyway, we had to dig hard for jokes.  
    	Even the dude I saw bathing in the Boulder Creek River got to put a joke in here.  Anyway, not to keep you 
    	waiting anymore, here is 'The Empire Strikes Back' starring Same Berger as Han Solo, Kevin Kirk as Darth Vader, 
    	Hunter Haller as Chewbacca, Max Hayne as Luke Skywalker and Krishna Adettiwar as The Emperor.";
    cout << "\n"  << endl;
    stringupdate();
